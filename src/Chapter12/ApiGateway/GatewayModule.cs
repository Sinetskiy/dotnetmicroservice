namespace ApiGateway
{
    ...
    {
        private static Product[] productList = new[]
        {
            new Product {ProductName = "Fancy shirt", ProductId = 1},
            new Product {ProductName = "Fancier shirt", ProductId = 2}
        };
        ...
        public GatewayModule(IHttpClientFactory clientFactory, ILogger logger)
        {
            ...
            Delete("/shoppingcart/{userid}", async parameters =>
            {
                ...
                HttpClient client =
                    await clientFactory.Create(new Uri("http://localhost:5200/"), "shopping_cart_write");
                var request = new HttpRequestMessage(HttpMethod.Delete, $"/shoppingcart/{userId}/items")
                {
                    Content = new StringContent(JsonConvert.SerializeObject(new[] {productId}), UTF8,
                        "application/json")
                };
        ...
        private List<Product> GetBasketProductsFromResponse(string responseBody)
        {
            return JsonConvert.DeserializeObject<ShoppingCart>(responseBody)
                       .Items
                       ?.Select(item =>
                           new Product {ProductName = item.ProductName, ProductId = item.productCatalogId })
